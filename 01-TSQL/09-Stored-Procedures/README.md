...
Neither topic is covered by the project's primary T-SQL reference
document, which either explicitly flags transaction management as
material for a separate chapter, or (in the case of TVPs) simply does
not mention the topic at all — these two files fill that gap.
